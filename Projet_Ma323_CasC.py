...
### Approximation de la solution exacte (avec Crank-Nicolson)

h_sol = 0.05
tau_sol = 0.00001

c_sol = nu*tau_sol/h_sol**2
d_sol = V*tau_sol/h_sol

CNg = matriceM(1 + c_sol, d_sol/4 - c_sol/2, -c_sol/2 - d_sol/4)
CNd = matriceM(1 - c_sol, -d_sol/4 + c_sol/2, c_sol/2 + d_sol/4)

def SolExacte(h_sol, tau_sol, CNg, CNd):
    ntfinal = int(Tmax/tau_sol)
    X = np.linspace(-10,10,N)
    T = np.arange(ntfinal + 1)*tau
    U = np.zeros((ntfinal, N)) # N est ici le même dans le cas d'étude et dans le test de précision
    U[0, : ] = U0(X)
    for i in range(ntfinal-1):
        U[ i+1, : ] = np.linalg.solve(CNg, CNd@U[ i , : ])
    return U


Sol = SolExacte(h_sol, tau_sol, CNg, CNd)

### Calcul de l'erreur

ErrEc = np.max(np.abs(Sol[-1, :] - Ue[-1, :]))
ErrEd = np.max(np.abs(Sol[-1, :] - Ued[-1, :]))
ErrIc = np.max(np.abs(Sol[-1, :] - Ui[-1, :]))
ErrId = np.max(np.abs(Sol[-1, :] - Uid[-1, :]))
ErrCNc = np.max(np.abs(Sol[-1, :] - Ucn[-1, :]))
ErrCNd = np.max(np.abs(Sol[-1, :] - U_cn[-1, :]))

print('Explicite centré erreur = ', ErrEc)
print('Explicite décentré erreur = ', ErrEd)
print('Implicite centré erreur = ', ErrIc)
print('Implicite décentré erreur = ', ErrId)
print('CN centré erreur = ', ErrCNc)
print('CN décentré erreur = ', ErrCNd)
